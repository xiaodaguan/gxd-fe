  ...
  <div>
    <a-menu :defaultSelectedKeys="['1']" :defaultOpenKeys="['sub1']" mode="inline" theme="dark" :inlineCollapsed="collapsed">
      ...
      <a-sub-menu v-for="item1 in hasChildrenList(menus)" :key="item1.name">
        ...
        >
        <!-- lv2 -->
        <a-menu-item v-for="item2 in noChildrenList(item1.children)" :key="item2.name" @click="handleClick(item2)">
          {{ item2.text }}
        </a-menu-item>
        <a-sub-menu v-for="item2 in hasChildrenList(item1.children)" :key="item2.name">
          ...
          >
          <!-- lv3 -->
          <a-menu-item v-for="item3 in noChildrenList(item2.children)" :key="item3.name" @click="handleClick(item3)">
            {{ item3.text }}
          </a-menu-item>
        </a-sub-menu>
      </a-sub-menu>
